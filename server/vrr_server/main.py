...
async def setup_frame_sending(width, height):
    ...
    ffmpeg_params = "-vaapi_device /dev/dri/renderD128 -vf 'format=nv12,hwupload' -c:v h264_vaapi -rc_mode CQP -qp 20 -coder cabac"
    ...
    ffmpeg_cmdlines = (
        f"ffmpeg -hwaccel vaapi -video_size 1366x768 -framerate 30 -f x11grab -i :0.0 {ffmpeg_params} -f rtsp -rtsp_transport udp rtsp://localhost:8554/screen_1.sdp",
